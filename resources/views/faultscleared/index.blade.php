...
<div class="container-fluid">
    ...
    <div class="row g-3 mb-4" data-aos="fade-up">
        <div class="col-xl-3 col-md-6">
            <div class="card shadow-sm border-0 rounded-4">
                <div class="card-body p-4 d-flex align-items-center">
                    ...
                    <div>
                        <div class="text-muted small mb-1">Total Cleared</div>
                        <h4 class="mb-0 fw-bold">{{ number_format($totalCleared) }}</h4>
                    </div>
                </div>
            </div>
        </div>
        <div class="col-xl-3 col-md-6">
            <div class="card shadow-sm border-0 rounded-4">
                <div class="card-body p-4 d-flex align-items-center">
                    <div class="flex-shrink-0 me-3">
                        <div class="rounded-circle d-flex align-items-center justify-content-center" style="width:50px;height:50px;background:rgba(25,135,84,0.18);">
                            <i class="fa-solid fa-bolt fs-5 text-success"></i>
                        </div>
                    </div>
                    <div>
                        <div class="text-muted small mb-1">Cleared &lt; 24h</div>
                        <h4 class="mb-0 fw-bold">{{ number_format($clearedUnder24h) }}</h4>
                    </div>
                </div>
            </div>
        </div>
        <div class="col-xl-3 col-md-6">
            <div class="card shadow-sm border-0 rounded-4">
                <div class="card-body p-4 d-flex align-items-center">
                    <div class="flex-shrink-0 me-3">
                        <div class="rounded-circle d-flex align-items-center justify-content-center" style="width:50px;height:50px;background:rgba(245,158,11,0.18);">
                            <i class="fa-solid fa-hourglass fs-5 text-warning"></i>
                        </div>
                    </div>
                    <div>
                        <div class="text-muted small mb-1">Cleared 24-48h</div>
                        <h4 class="mb-0 fw-bold">{{ number_format($cleared2448h) }}</h4>
                    </div>
                </div>
            </div>
        </div>
        <div class="col-xl-3 col-md-6">
            <div class="card shadow-sm border-0 rounded-4">
                <div class="card-body p-4 d-flex align-items-center">
                    ...
                    <div>
                        <div class="text-muted small mb-1">Svc-Affecting</div>
    ...
    <div class="card shadow-sm border-0 rounded-4 overflow-hidden" data-aos="fade-up">
        <div class="card-header bg-white border-0 d-flex justify-content-between align-items-center flex-wrap gap-3 pt-4 px-4 pb-3">
            <div>
                <h5 class="mb-1 fw-bold text-dark"><i class="fa-solid fa-circle-check me-2 text-success"></i>{{ $pageTitle }}</h5>
                <p class="text-muted small mb-0">Faults cleared in 24 &amp; 48 hours — closed tickets</p>
            </div>
            <div class="d-flex align-items-center gap-2 flex-wrap">
                <a href="{{ route('faultscleared.create') }}" class="btn btn-sm btn-primary fw-semibold">
                    <i class="fa-solid fa-plus me-1"></i> Add Cleared Fault
                </a>
            ...
            <form method="GET" class="auto-filter-form d-flex gap-2 flex-wrap align-items-end">
                <div class="flex-grow-1" style="min-width:260px;">
                    ...
                    <div class="input-group">
                        ...
                        <input type="text" name="search" class="form-control" placeholder="Search Customer, City, Issue, Owner, RCA, RFO..." value="{{ $search }}">
                    </div>
                </div>
                ...
                <a href="{{ route('faultscleared.index') }}" class="btn btn-outline-secondary btn-sm">
            ...
            <table class="table table-hover align-middle mb-0" style="min-width:1400px;">
                <thead class="bg-light">
                    <tr>
                        ...
                        <th class="px-4 py-3 text-uppercase small text-muted">Closed</th>
                ...
                <tbody>
                    @forelse($faultsCleared as $i => $f)
                    <tr>
                        <td class="px-4 py-2 small">{{ $faultsCleared->firstItem() + $i }}</td>
                        ...
                        <td class="px-4 py-2 small">
                            <span class="text-success fw-semibold">{{ $f->closed_at ? $f->closed_at->format('d M Y') : '—' }}</span>
                        </td>
                        <td class="px-4 py-2 small" style="max-width:260px;">{{ \Illuminate\Support\Str::limit($f->issue, 65) }}</td>
                        <td class="px-4 py-2 small text-muted">{{ $f->owner ?? '—' }}</td>
                    ...
                    <tr>
                        <td colspan="10" class="text-center py-5 text-muted">
                            ...
                            No Faults Cleared data. Import from the <a href="{{ route('imports.index') }}">Data Import page</a>.
                        </td>
                    </tr>
                    @endforelse
                </tbody>
        ...
        @include('components.pagination', ['paginator' => $faultsCleared])
    </div>
</div>
@endsection
