...
class FaultReportedImport implements ToModel, WithStartRow, WithBatchInserts, WithChunkReading, SkipsOnError, SkipsOnFailure
{
    use SkipsFailures;
    ...
    public function model(array $row)
    {
        ...
        $agingDowntime    = $this->cleanValue($row[9] ?? null);
        $rfo              = $this->cleanValue($row[10] ?? null);
        $rca              = $this->cleanValue($row[11] ?? null);

        if (empty($customerName)) {
            return null;
        }

        FaultReported::updateOrCreate(
            [
